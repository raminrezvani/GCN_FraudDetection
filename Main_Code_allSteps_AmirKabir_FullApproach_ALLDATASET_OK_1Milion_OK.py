...
MAX_TRANSACTIONS = 1_000_000  # Limit to 1 million transactions
...
def compute_hmm_based_embeddings(graph, primary_to_idx, second_to_idx, transactions_path="Transactions.csv"):
    print("Step 1: Computing HMM-based embeddings with degree centrality only...")
    ...
    centrality_measures = [nx.degree_centrality]
    num_features = len(centrality_measures)  # Now 1

    feature_matrix = np.zeros((num_nodes, num_features, len(graphs)))

    for t, G in enumerate(graphs):
        print(f"Computing degree centrality for window {t + 1}/{len(graphs)}...")
        ...
        for node in centrality:
            ...
            if idx >= num_nodes:
                continue
            feature_matrix[idx, 0, t] = centrality[node]

    hmm_embeddings = {}
    for node_idx in range(num_nodes):
        node_features = feature_matrix[node_idx]
        hmm_scores = []
        sequence = node_features[0].reshape(-1, 1)
...
# DataLoader for Custom Dataset (Updated to Sample 1M Transactions)
class DataLoader:
    @staticmethod
    def load_custom_data(transactions_path="Transactions.csv", labels_path="Second_ID_Label.csv"):
        print("Loading custom dataset with 1M random transactions including labeled ones...")
        df_labels = pd.read_csv(labels_path)
        labeled_second_ids = set(df_labels['Second_ID'].values)

        # Load transactions and filter for labeled Second_IDs
        df = pd.read_csv(transactions_path)
        print(f"Total transactions in file: {len(df)}")

        # Split transactions into labeled and unlabeled
        labeled_df = df[df['Second_ID'].isin(labeled_second_ids)]
        unlabeled_df = df[~df['Second_ID'].isin(labeled_second_ids)]

        print(f"Labeled transactions: {len(labeled_df)}, Unlabeled transactions: {len(unlabeled_df)}")

        # If labeled transactions exceed 1M, sample them; otherwise, keep all and sample from unlabeled
        if len(labeled_df) >= MAX_TRANSACTIONS:
            sampled_df = labeled_df.sample(n=MAX_TRANSACTIONS, random_state=42)
        else:
            remaining_slots = MAX_TRANSACTIONS - len(labeled_df)
            if remaining_slots > len(unlabeled_df):
                sampled_unlabeled_df = unlabeled_df
            else:
                sampled_unlabeled_df = unlabeled_df.sample(n=remaining_slots, random_state=42)
            sampled_df = pd.concat([labeled_df, sampled_unlabeled_df])

        sampled_df = sampled_df.drop_duplicates(subset=['Primary_ID', 'Second_ID'])
        print(f"Sampled transactions: {len(sampled_df)}")

        # Build graph with sampled data
        data = {
            'Primary_ID': list(sampled_df['Primary_ID'].values),
            'Second_ID': list(sampled_df['Second_ID'].values),
        }
...
def main():
    ...
    num_nodes = graph.num_nodes()
    combined_embeddings = np.zeros((num_nodes, 16))  # 1D HMM + 5D FraudWalk + 10D GCN
    for i in range(num_nodes):
        hmm = hmm_embeddings.get(i, [0])  # 1D due to degree centrality only
