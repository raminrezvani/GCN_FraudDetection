...
def compute_centrality_features(graph):
    ...
    G_nx = nx.DiGraph()
    src, tgt = graph.edges()  # Single relation type, no etype needed
...
def compute_fraudwalk_embeddings(graph, train_mask=None, inductive_nodes=None):
    ...
    src, tgt = graph.edges()  # Single relation type
    edge_list = list(zip(src.numpy(), tgt.numpy()))
    G = nx.DiGraph()
...
class DataLoader:
    @staticmethod
    def load_custom_data(transactions_path="Transactions_1M.csv", labels_path="Second_ID_Label.csv"):
        ...
        # Create DGL graph (homogeneous)
        graph = dgl.graph((edge_index[0], edge_index[1]), num_nodes=num_nodes)

        # Features (placeholder: degree will be computed later, start with zeros)
        features = torch.zeros((num_nodes, 1), dtype=torch.float)

        # Labels: Apply to Second_ID nodes, -1 for Primary_ID nodes
...
# Main Function
def main():
    graph, features, labels, num_classes = DataLoader.load_custom_data(
