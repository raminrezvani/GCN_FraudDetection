...
NUM_RELATIONS = 3  # Matches Amazon's 3 relation types (U-P-U, U-S-U, U-V-U)
...
def compute_centrality_features(graph):
    ...
    G_nx = nx.DiGraph()
    src, tgt = graph.edges(etype=graph.etypes[0])  # Use first relation type
...
# DataLoader for Amazon-Fraud Dataset
class DataLoader:
    @staticmethod
    def load_amazon_fraud_data():
        print("Loading Amazon-Fraud dataset from DGL...")
        dataset = dgl.data.fraud.FraudAmazonDataset()
        graph = dataset[0]  # First graph in the dataset
        num_classes = dataset.num_classes
        features = graph.ndata['feature'].float()  # Node features
        labels = graph.ndata['label'].long()  # Binary labels (0 or 1, possibly -1 for unlabeled)
...
class GraphProcessor:
    ...
    def convert_to_pyg_format(graph, features):
        ...
        edge_type = edge_types[0]  # Use first relation type
...
# Main Function
def main():
    graph, features, labels, num_classes = DataLoader.load_amazon_fraud_data()
    ...
    combined_embeddings = np.zeros((num_nodes, 23))  # 8D centrality + 5D fraudwalk + 10D GCN
    ...
    model = CombinedModel(
        input_dim=23,  # Combined embedding size
